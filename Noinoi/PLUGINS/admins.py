...
async def skip(client, m: Message):
    ...
    if len(m.command) < 2:
        ...
        if op == 0:
            ...
            await m.reply("💬 **Have no playlist assistanat leaving voice chat**")
        elif op == 2:
            await m.reply("💬 **Clearing the songs userbot leaving voice chat**")
        else:
            await m.reply_photo(
                photo=f"{IMG_3}",
...
async def unmute(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await m.reply(
                "💬 **Userbot unmuted To mute the userbot, use the**\n» /mute **command**."
            )
        except Exception as e:
            await m.reply(f"💬 **error:**\n\n`{e}`")
    else:
...
async def cbresume(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        try:
            await call_py.resume_stream(chat_id)
            await query.edit_message_text(
                "💬 the streaming has resumed", reply_markup=bttn
            )
        except Exception as e:
            await query.edit_message_text(f"💬 **error:**\n\n`{e}`", reply_markup=bcl)
    else:
